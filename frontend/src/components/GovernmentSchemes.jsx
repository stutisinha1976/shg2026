...
export default function GovernmentSchemes({ schemes }) {
  ...
  return (
    <div className="schemes-card glass-card">
      <h3 className="section-title"><span className="icon">🏛️</span>Government Scheme Eligibility</h3>
        ...
        {entries.map(([member, memberSchemes]) => (
          <div key={member} className="schemes-member-block">
            <div className="schemes-member-header">
              <h4>{member}</h4>
              <span className="schemes-count">{memberSchemes.length} schemes</span>
            </div>
              ...
              {memberSchemes.map((s, i) => (
                <div key={i} className="scheme-item">
                  <div className="scheme-header">
                    ...
                    <h5 className="scheme-name">{s.scheme_name}</h5>
                  </div>
                  <p className="scheme-description">{s.description}</p>
                  <div className="scheme-meta">
                    <span className="scheme-tag">Max: {s.max_amount}</span>
                    <span className="scheme-tag">Rate: {s.rate}</span>
                  </div>
                </div>
              ))}
            </div>
